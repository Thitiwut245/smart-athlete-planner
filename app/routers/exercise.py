...
ALLOWED_MUSCLES = {
    ...
    "quadriceps","traps","triceps","shoulders","obliques"
}
...
def _search_remote(muscle: Optional[str], type: Optional[str]) -> list:
    qs = []
    if muscle:
...
def _search(muscle: Optional[str], type: Optional[str]) -> tuple[list, str]:
    """
    Returns (data, source) where source in {'remote','fallback','cache'}.
    Applies in-memory cache and fallback JSON.
    """
    key = f"{muscle or ''}|{type or ''}"
    ...
    try:
        data = _search_remote(muscle, type)
        _cache_set(key, data)
        return data, "remote"
...
@router.get("", summary="Search exercises by muscle/type")
def search(muscle: Optional[str] = None, type: Optional[str] = None, response: Response = None):
    if muscle and muscle not in ALLOWED_MUSCLES:
        raise HTTPException(400, f"Unsupported muscle '{muscle}'")
    data, source = _search(muscle, type)
    if response is not None:
        response.headers["X-Exercise-Source"] = source
    return data


@router.get("/random", summary="Get a random exercise by muscle/type")
def random_ex(muscle: Optional[str] = None, type: Optional[str] = None, response: Response = None):
    if muscle and muscle not in ALLOWED_MUSCLES:
        raise HTTPException(400, f"Unsupported muscle '{muscle}'")
    data, source = _search(muscle, type)
    if not data:
